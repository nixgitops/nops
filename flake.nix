  ...
  in {
    ...
    in {
      ...
      options.services.nops = {
        ...
        webhook = {
          ...
          sslCert = lib.mkOption {
            ...
            default = "/home/nops/certs/server.crt";
            description = "Path to the SSL certificate file for the webhook listener. Set both sslCert and sslKey to null to opt into HTTP-only mode.";
          };
          sslKey = lib.mkOption {
            type = lib.types.nullOr lib.types.str;
            default = "/home/nops/certs/server.key";
            description = "Path to the SSL private key file for the webhook listener. Set both sslCert and sslKey to null to opt into HTTP-only mode.";
          };
        };
      };
      ...
      config = lib.mkIf cfg.enable {
        assertions = [
          {
            assertion = (cfg.webhook.sslCert == null) == (cfg.webhook.sslKey == null);
            message = "services.nops.webhook.sslCert and services.nops.webhook.sslKey must either both be set or both be null. Set both to null only for intentional HTTP-only webhook mode.";
          }
        ];
        ...
        systemd.tmpfiles.rules = [
          ...
          "d /home/nops/certs 0700 nops nops -"
        ];
        ...
        systemd.services.nops = {
          ...
          # Generates a self-signed TLS cert at the configured path if webhook TLS is enabled and no cert exists yet.
          preStart = ''
            if [ "${builtins.toString cfg.webhook.enable}" = "1" ] \
              && [ -n "${if cfg.webhook.sslCert != null then cfg.webhook.sslCert else ""}" ] \
              && [ -n "${if cfg.webhook.sslKey != null then cfg.webhook.sslKey else ""}" ]; then
